...
namespace Kinect.Toolbox {
  public class SkeletonDisplayManager {
    ...
    void Plot(JointType centerID, IEnumerable<Joint> joints) {
      ...
      GetCoordinates(centerID, joints, out centerX, out centerY, out state);
      ...
      Ellipse ellipse = new Ellipse {
        ...
        StrokeThickness = 4.0,
        Stroke = state == JointTrackingState.Inferred ? new SolidColorBrush(Colors.Yellow) :  
    ...
    void Trace(JointType sourceID, JointType destinationID, JointCollection joints) {
      ...
      float sourceY;
      JointTrackingState state;
      GetCoordinates(sourceID, joints, out sourceX, out sourceY, out state);

      float destinationX;
      float destinationY;

      GetCoordinates(destinationID, joints, out destinationX, out destinationY, out state);
